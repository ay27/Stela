...
- `search_tables` ranks the live connector catalog by table name plus column COMMENT, the latter via the connector's optional `describeTables` API (one batched call per lookup, see [ADR-0042](./adr/0042-connector-describe-tables-api.md)). Catalog enumeration lists databases concurrently with at most four in-flight `listTables` calls; results remain uncached so each lookup reflects the live connector. CJK runs are expanded into bigrams so Chinese business terms match. Each candidate also carries `vaultUsage` (notes, blocks, last run date) as information for the model — usage never enters the score. `get_table_schema` probes a qualified `database.table` directly without enumerating the catalog; only unqualified table names enumerate it to resolve the database. Multiple requested tables are probed concurrently. ([ADR-0041](./adr/0041-agent-live-schema-authority.md)) Its result carries every column as a `name:type` line with per-table completeness flags; comments ride along only for explicitly named columns, and DDL is opt-in because it re-derives the same list ([ADR-0082](./adr/0082-coverage-over-precision-in-bounded-tool-results.md)).
