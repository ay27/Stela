...
- `search_tables` ranks the `schemaDir` catalog by table name 16, column name 8, column `COMMENT` 6, DDL fallback 1. CJK runs are expanded into bigrams so Chinese business terms match. Each candidate also carries `vaultUsage` (notes, blocks, last run date) as information for the model — usage never enters the score.
