...
if (!gotLock) {
  ...
  // 退出时先 flush AutoGit checkpoint，再按顺序释放 native watcher、connector
  // 与结果缓存。每步都打计时日志：退出路径一旦卡住，终端里最后一条 quit
  // 日志就是卡住的阶段。
  ...
  app.on("before-quit", (event) => {
    ...
    void (async () => {
      try {
        ...
        // Electron 41 的 process.exit/app.exit 会继续走异步应用退出。这里仍保留
        // 原来的底层快速出口，但必须等 @parcel/watcher.unsubscribe() 完成，避免
        // Node 析构时 FSEventsBackend 原生线程仍在运行并触发 SIGABRT。
        log.info("quit: forcing process exit", { elapsed: elapsed() });
        const reallyExit = (process as typeof process & {
          reallyExit?: (code: number) => void;
        }).reallyExit;
        if (reallyExit) reallyExit(0);
        else app.exit(0);
      }
    })();
  });
