/**
 * `runsql` fence 的解析 / detail 补丁——纯字符串逻辑，main / renderer 共用的规范
 * 实现。`src/editor/runsql/markdown-patch.ts` 直接重导出本模块。
 *
 * main 侧的 SQL 索引服务（`electron/services/sql-index.ts`）用 `parseRunsqlFences`
 * 从 vault 里每个 `.md` 文件切出所有 runsql 块（连同其后跟随的 `<detail>` 摘要），
 * 交给 SQL 事实抽取器解析。
 */
import type { DetailMeta } from "./detail-meta";
