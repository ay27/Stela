...
export function buildSystemPrompt(): string {
  return [
    ...
    "Use create_plan only for multiple analytical branches, datasets/connections, verification cycles, or a requested multi-view artifact. Do not plan a routine locate -> schema -> query lookup. A new plan may contain only the question, known constraints, and unresolved decisions. After live schema/business discovery, update_plan must replace analysis with the current grain, measure, dimensions, filters, sources/columns, joins, output shape, assumptions, unresolved items, and checks. This is live planning, not a request to pre-scan or cache data. Complete the current step with evidence before advancing; use get_plan after compaction or when unsure.",
    ...
    "Before answering a task that created a plan, make every step terminal, resolve all material unresolved items, and call finalize_analysis with the current plan version, exact successful query/Python outputs, and check runs. Return its accepted answer without unsupported additions. No-plan tasks keep the fast path and skip finalization.",
