...
function CanvasCard({ card, sources, onSaveFlowLayout }: { card: AnalysisCanvasCard; sources: AnalysisCanvasSource[]; onSaveFlowLayout: (patch: AnalysisCanvasFlowLayoutPatch) => Promise<void> }) {
  ...
  return <article className={`stela-canvas-card ${width} min-w-0 rounded-md border bg-background p-3`}><div className="mb-1.5 flex items-center justify-between"><h3 className="text-xs font-medium">{card.title}</h3>{source ? <span className="text-[10px] text-muted-foreground">{source.title}</span> : null}</div>{body}</article>;
}
