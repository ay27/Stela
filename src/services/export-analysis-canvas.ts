...
import { layoutFlowCard, measureFlowNode, type FlowCard } from "@/components/flow/flow-layout";
...
async function renderFlowCard(card: FlowCard): Promise<string> {
  ...
  const sizedNodes = nodes.map((node) => ({ ...node, size: measureFlowNode(node) }));
  ...
  const shifted = new Map(sizedNodes.map((node) => [node.id, {
    ...node,
    position: { x: node.position!.x - minX + padding, y: node.position!.y - minY + padding },
  }]));
  const edges = card.edges.map((edge) => {
    const source = shifted.get(edge.source);
    const target = shifted.get(edge.target);
    ...
    const path = horizontal ? `M ${sx} ${sy} C ${middle} ${sy}, ${middle} ${ty}, ${tx} ${ty}` : `M ${sx} ${sy} C ${sx} ${middle}, ${tx} ${middle}, ${tx} ${ty}`;
    const tone = edge.tone ? ` flow-edge-${edge.tone}` : "";
    const label = edge.label ? `<text x="${(sx + tx) / 2}" y="${(sy + ty) / 2 - 6}" text-anchor="middle">${esc(edge.label)}</text>` : "";
    return `<path class="flow-edge${tone}" d="${path}" marker-end="url(#${markerId})"/>${label}`;
  }).join("");
  const nodeHtml = [...shifted.values()].map((node) => `<div class="flow-node ${flowTone(node.tone, node.kind)} ${node.kind === "decision" ? "flow-decision" : node.kind === "note" ? "flow-note" : ""}" style="left:${node.position.x}px;top:${node.position.y}px;width:${node.size.width}px;height:${node.size.height}px"><strong>${esc(node.label)}</strong>${node.description ? `<small>${esc(node.description)}</small>` : ""}</div>`).join("");
  return `<div class="flow-scroll"><div class="flow" style="width:${width}px;height:${height}px"><svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><marker id="${markerId}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z"/></marker></defs>${edges}</svg>${nodeHtml}</div></div>`;
}
